...
/// proving the API wiring (paths, parsing, refusal handling) before the tag.
///
/// NOT part of CI — runs only under the flag:
///
///     flutter test --dart-define=FUFUT_PARITY_SMOKE=true test/parity_smoke_test.dart
library;
...
void main() {
  const live = bool.fromEnvironment('FUFUT_PARITY_SMOKE');

  test('parity API surface: chef + waiter probes', () async {
    if (!live) {
      markTestSkipped('live probe — run with --dart-define=FUFUT_PARITY_SMOKE=true');
      return;
    }
  ...
  // ── Head chef: the stock chain + reports extras ──────────────────────────
  {
    ...
    check('GET shifts tolerates the manager-only 403', shifts.isEmpty,
        '${shifts.length} rows for chef (correct: manager-only)');
    final staff = await api.staff();
    check('GET staff tolerates the 403 (manager-only roster)',
        true, '${staff.length} rows');
  ...
  // ── Head waiter: reservations book + refuse paths ────────────────────────
  {
    ...
    final audit = await api.auditFiltered(
        actorId: me, from: '2026-09-21T00:00:00');
    check('GET audit (actor_id=me)', true, '${audit.length} entries');
